...
class GameManager:
  def __init__(self, width=500, height=500, squares_count=16, gap=50):
    ...
    # sanity check
    # assert self.MAX_GUESSES == 13 # not true for all n, is true for n=16
    assert width == height
    assert width % 2 == 0
    assert gap % 2 == 0
  ...
  # returns the Text
  def reveal_and_return(self, data):
  ...
  def generate_gutter_text(self, text, gutter_constant):
    text = Text(\
      Point(\
        int(math.ceil(self.WIDTH / 2)), \
        int(math.ceil(self.HEIGHT * (0.25 * gutter_constant)))), \
        text)
  ...
  def game_loop(self):
    ...
    # if either loop conditions are broken then we can identify a win or loss based on whether there are detectable squares left
    if len(self.SQUARES) == 0:
      outcome = self.generate_gutter_text('You Win!', 2)
